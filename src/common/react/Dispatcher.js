import { isFunction, isDepsChanged } from './utils';
import { MEMO, EFFECT, LAYOUT_EFFECT } from './constants';
...
export default class Dispatcher {
  ...
  update(unmount) {
    ...
    unmount ?
      ...
      Promise.resolve().then(() => this.runEffects(EFFECT));
  }
    ...
    const [redux] = this.useState(() => {
      ...
      return {
        ...
        dispatch: (action) => { // never update
          ...
          !Object.is(value.state, newState) && setter({ ...value, state: newState });
        }
      };
    });
